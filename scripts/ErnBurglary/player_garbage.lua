...
local persistentState = {
    ...
    -- spottedByFactionID is a map of factionID -> list of actor IDs
    spottedByFactionID = {},
    ...
    -- picks the item up.
    itemsInCellOwnership = {},
    -- Need this reverse lookup because of the shape of ownership info.
    actorRecordIDtoInstanceID = {}
}
...
local function anyNPCsAlive(listOfNPCs)
    ...
    for _, npcInstance in ipairs(listOfNPCs) do
        if types.Actor.objectIsInstance(npcInstance) ~= true then
...
local function calculateTheft()
    ...
    for _, item in ipairs(types.Actor.inventory(self):getAll()) do
        local owner = persistentState.itemsInCellOwnership[item.id]
        if (owner ~= nil) then
            ...
            -- double jeopardy.
            if (owner.recordId ~= nil) and (persistentState.spottedByActorRecordID[owner.recordId]) and
                anyNPCsAlive({persistentState.actorRecordIDtoInstanceID[owner.recordId]}) then
            ...
            elseif (owner.factionId ~= nil) and (atLeastRank(owner.factionId, owner.factionRank) ~= true) then
                ...
                if anyNPCsAlive(persistentState.spottedByFactionID[owner.factionId]) then
...
local function cellChanged()
    ...
    -- reset for new cell.
    persistentState.lastCellID = self.cell.id
    persistentState.spottedByActorRecordID = {}
    persistentState.actorRecordIDtoInstanceID = {}
    persistentState.spottedByFactionID = {}
...
local function ownershipInfo(data)
    ...
    if data.cellID == self.cell.id then
        persistentState.itemsInCellOwnership = data.itemIDtoOwnership
    end
...
local function oldonUpdate(dt)
    if persistentState.lastCellID ~= self.cell.id then
        settings.debugPrint("cell changed from " .. tostring(lastCellID) .. " to " .. self.cell.id)
        -- cell change
        cellChanged()
    end
    ...
    for _, actor in ipairs(nearby.actors) do
        -- check for detectiong
        if persistentState.spottedByActorRecordID[actor.id] == nil then
            local isActive = core.sound.isSayActive(actor)
            if isActive then
                ...
                if actorRecord ~= nil then
                    ...
                    persistentState.spottedByActorRecordID[actor.id] = true
                    persistentState.spottedByActorRecordID[actorRecord.id] = true
                    persistentState.actorRecordIDtoInstanceID[actorRecord.id] = actor.id
                    ...
                    for _, factionId in pairs(types.NPC.getFactions(actor)) do
                        if persistentState.spottedByFactionID[factionId] == nil then
                            persistentState.spottedByFactionID[factionId] = {}
                        end
                        table.insert(persistentState.spottedByFactionID[factionId], actor.id)
                        settings.debugPrint(
